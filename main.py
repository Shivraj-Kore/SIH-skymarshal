...
class LaserTracker(object):
    ...
    def __init__(self, cam_width=640, cam_height=480, hue_min=20, hue_max=160,
        ...
        * ``display_thresholds`` -- if True, additional windows will display
        ...
        self.trail = numpy.zeros((self.cam_height, self.cam_width, 3),
                                 numpy.uint8)
        self.first_detection_frame_index = None
        self.first_detection_coordinates = None
    ...
    def track(self, frame, mask):
        ...
        # Resize the frame to match the size of self.trail
    ...
    def detect(self, frame, frame_index):
        ...
        # Track the laser and get its coordinates
        coordinates = self.track(frame, self.channels['laser'])

        # Check for the first detection
        if self.first_detection_frame_index is None and cv2.countNonZero(self.channels['laser']) > 0:
            ...
            file = open('frame_index.txt', 'w')
            file.writelines(str(frame_index))
            file.close()
            cv2.imwrite(f'frame_{frame_index}.png', frame)
            # cv2.imwrite(f'laser_ptr_image_{frame_index}.png', self.channels['laser'])
